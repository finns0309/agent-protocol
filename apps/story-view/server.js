...
const PORT = Number(process.env.STORY_VIEW_PORT || 4186);
const NETWORK_API_PORT = Number(process.env.NETWORK_API_PORT || process.env.NETWORK_SERVER_PORT || 4311);
const NETWORK_API_BASE = process.env.NETWORK_API_BASE || "";
const BIND_HOST = process.env.STORY_BIND_HOST || "";
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/config") {
      return sendJson(res, 200, { networkApiBase: resolveNetworkApiBase(req), observerMode: "story" });
    }
...
server.listen(PORT, BIND_HOST || undefined, () => {
  console.log(`[story-view] http://127.0.0.1:${PORT}`);
});
