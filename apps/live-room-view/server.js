...
const PORT = Number(process.env.LIVE_ROOM_VIEW_PORT || 4187);
...
const BIND_HOST = process.env.LIVE_ROOM_BIND_HOST || "";
const PUBLIC_HOST = process.env.POLIS_PUBLIC_HOST || process.env.AGORA_PUBLIC_HOST || "";
...
function getDisplayHost(req) {
  ...
  return PUBLIC_HOST || requestHost || "127.0.0.1";
}
...
server.listen(PORT, BIND_HOST || undefined, () => {
  ...
  console.log(`[live-room-view] observer server listening on ${localUrl}`);
  if (PUBLIC_HOST) {
    console.log(`[live-room-view] share on local network: ${publicUrl}`);
  }
});
