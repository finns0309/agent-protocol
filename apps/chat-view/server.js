...
const PORT = Number(process.env.CHAT_VIEW_PORT || process.env.HUMAN_UI_PORT || 4181);
...
server.listen(PORT, POLIS_BIND_HOST || undefined, () => {
  ...
  console.log(`[chat-view] observer server listening on ${localUrl}`);
  if (POLIS_PUBLIC_HOST) {
    console.log(`[chat-view] share on local network: ${publicUrl}`);
  }
});
