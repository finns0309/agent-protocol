...
export function createSidebar(session, state, setState) {
  const onlineAgents = Array.isArray(session.agents) ? session.agents.length : 0;
  const workspaceName = session.workspace?.name || "Polis";
  const activeSessionMeta = Array.isArray(state.sessions)
    ? state.sessions.find((entry) => entry.id === (state.resolvedSessionId || state.sessionId))
    : null;
  const sessionName =
    activeSessionMeta?.name || session.session?.name || state.resolvedSessionId || session.session?.id || "";
  const aside = document.createElement("aside");
  aside.className = "sidebar";
  aside.innerHTML = `
    <div class="sb-h">
      <div class="sb-title-group">
        <span class="sb-name">${escapeHtml(workspaceName)}</span>
        <span class="sb-sub">${escapeHtml(sessionName || "observer workspace")}</span>
      </div>
      <div class="sb-logo" aria-hidden="true">#</div>
    </div>

    <div class="sb-search">Search channels or traces</div>
    ...
    <div class="sb-f">
      <div class="sb-ft">
        ...
        protocol v0.2 · ${escapeHtml(onlineAgents)} agents online
      </div>
    </div>
  `;
