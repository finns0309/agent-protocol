...
const PORT = Number(process.env.POLIS_VIEW_PORT || 4281);
...
server.listen(PORT, POLIS_BIND_HOST || undefined, () => {
  ...
  console.log(`[polis-view] observer server listening on ${localUrl}`);
  if (POLIS_PUBLIC_HOST) {
    console.log(`[polis-view] share on local network: ${publicUrl}`);
  }
});
