...
function createEmptyState(channel) {
  const empty = document.createElement("div");
  empty.className = "channel-empty-state";
  empty.innerHTML = `
    <div class="channel-empty-label">No activity yet</div>
    <div class="channel-empty-text">${channel.topic || "This channel is waiting for the next message."}</div>
  `;
  return empty;
}
...
function renderTimeline(feed, channel, state, setState, systemNode = null) {
  ...
  if (!timeline.length) {
    const emptyState = createEmptyState(channel);
    if (cursor) {
      feed.insertBefore(emptyState, cursor);
    } else {
