...
const PORT = Number(process.env.OPS_VIEW_PORT || 4182);
...
server.listen(PORT, POLIS_BIND_HOST || undefined, () => {
  ...
  console.log(`[ops-view] observer server listening on ${localUrl}`);
  if (POLIS_PUBLIC_HOST) {
    console.log(`[ops-view] share on local network: ${publicUrl}`);
  }
});
